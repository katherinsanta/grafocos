...
{
	public class FiltroManguera : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.DropDownList ddlManguera;
		...
		private bool VerificarPermisos()
		{
			...
			permiso = ((Libreria.Usuario)Session["Usuario"]).Permisos.Verificar(modulo, opcion, "Ventas Manguera");
		...
		private void EjecutarReporte()
		{
			if (this.Validar())
			{
				try
				{
					Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,323, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();	
					DataDynamics.ActiveReports.ActiveReport report = null;
					if (this.TipoReporte.SelectedValue == "Detallado")
                        report = new Servipunto.Estacion.Web.Modules.Rpt.VentasMangueraDetallado(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text), int.Parse(this.ddlManguera.SelectedValue));
					else if (this.TipoReporte.SelectedValue == "Resumido")
                        report = new Servipunto.Estacion.Web.Modules.Rpt.VentasMangueraResumido(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text), int.Parse(this.ddlManguera.SelectedValue));
		...
		{
            #region traduccion
            this.lblReporte.Text = "<b>" + Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(20, Global.Idioma) + "</b>";
		...
		#region Cargar mangueras
		private void CargarManguera()
		{
			for (int i = 1; i <= 40; i++)
			{
				this.ddlManguera.Items.Add(new ListItem(i.ToString(), i.ToString()));
			}
            this.ddlManguera.Items.Insert(0, new ListItem(Libreria.Configuracion.PalabrasIdioma.Traduccion(82, Global.Idioma), "0"));
		}
		#endregion
