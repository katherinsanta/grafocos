...
{
	public class Capturadores : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack) 
			{
				...
				{
					Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,62, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
					this.Visualizar();
				}
			}
			else
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
				this.AgregarAccion(ref htmlAcciones, "Capturador.aspx", Libreria.Configuracion.PalabrasIdioma.Traduccion(1108,Global.Idioma));
			if (eliminar)
                this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Libreria.Configuracion.PalabrasIdioma.Traduccion(1119, Global.Idioma));
		...
		private void Visualizar()
		{
			try
			{
				Libreria.Configuracion.Capturadores objCapturadores = new Libreria.Configuracion.Capturadores();
				Results.DataSource = objCapturadores;
				Results.DataBind();
				objCapturadores = null;
			}
			catch(Exception ex)
			{
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.Capturadores.Eliminar(Convert.ToInt16(aSelectedItems[i]));
					}
					catch (Exception ex)
					{
