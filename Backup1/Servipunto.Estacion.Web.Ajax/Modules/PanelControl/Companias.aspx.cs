...
{
	public class Companias : System.Web.UI.Page
	{
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
				this.AgregarAccion(ref htmlAcciones, "Compania.aspx", Libreria.Configuracion.PalabrasIdioma.Traduccion(1080,Global.Idioma));
			if (eliminar)
				this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Libreria.Configuracion.PalabrasIdioma.Traduccion(1081,Global.Idioma));
		...
		{
			Libreria.Configuracion.Companias objCompanias = new Libreria.Configuracion.Companias();
			Results.DataSource = objCompanias;
			Results.DataBind();
			objCompanias = null;
		}
		#endregion
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.Companias.Eliminar(Convert.ToInt16(aSelectedItems[i]));
					}
					catch (Exception ex)
					{
