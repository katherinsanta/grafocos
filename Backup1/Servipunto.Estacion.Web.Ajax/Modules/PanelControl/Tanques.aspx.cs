...
{
	public class Tanques : System.Web.UI.Page
	{
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
                this.AgregarAccion(ref htmlAcciones, "Tanque.aspx", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1383, Global.Idioma));
			if (eliminar)
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.Tanques.Tanques.Eliminar(int.Parse(aSelectedItems[i]));
					}
					catch (Exception ex)
					{
