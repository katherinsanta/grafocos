...
{
	public class Surtidores : System.Web.UI.Page
	{
		...
		private Libreria.Configuracion.Isla _objIsla = null;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
            ...
            #endregion
            if (Request.QueryString["IdIsla"] == null && lblHide.Text == "")
            {
                this.SetError("Navegación Invalida!", true);
                return;
            }
            else if ( lblHide.Text == "")
                lblHide.Text = Request.QueryString["IdIsla"].ToString();

			if (!this.Page.IsPostBack) 
			{
				if (this.VerificarPermisos())
				{
					Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,86, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
					this.Visualizar();
				}
			}
			else
		...
		private bool ObtenerObjetoIsla()
		{
			try
			{
                this._id = DecryptText(lblHide.Text.Replace(" ", "+"));//DecryptText(Convert.ToString(Request.QueryString["IdIsla"].Replace(" ","+") ));
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
                this.AgregarAccion(ref htmlAcciones, "Surtidor.aspx?IdIsla=" + Request.QueryString["IdIsla"], Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1386, Global.Idioma));
			if (eliminar)
                this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1387, Global.Idioma));

            this.AgregarAccion(ref htmlAcciones, "Islas.aspx", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1000, Global.Idioma));
		...
		private void Visualizar()
		{
			try
			{
				...
				{
                    this.lblTitle.Text = Libreria.Configuracion.PalabrasIdioma.Traduccion(1724, Global.Idioma) + ": <b>" + this._objIsla.IdIsla + "</b>";

					Libreria.Configuracion.Surtidores objSurtidores = new Libreria.Configuracion.Surtidores(this._objIsla.IdIsla, true, false);
					Results.DataSource = objSurtidores;
					Results.DataBind();
					objSurtidores = null;
				}
			}				
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,89, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.Surtidores.Eliminar(Convert.ToInt16(aSelectedItems[i]));
					}
					catch (Exception ex)
					{
