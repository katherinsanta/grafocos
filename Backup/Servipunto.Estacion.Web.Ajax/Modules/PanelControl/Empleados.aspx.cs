...
{
	public class Empleados : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack) 
			{
				...
				{
					Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,116, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
					this.Visualizar();
				}
			}
			else
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
                this.AgregarAccion(ref htmlAcciones, "Empleado.aspx", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1129, Global.Idioma));
			if (eliminar)
                this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1135, Global.Idioma));
		...
		private void Visualizar()
		{
			try
			{
				Libreria.Configuracion.Empleados objEmpleados = new Libreria.Configuracion.Empleados();
		...
		private void Eliminar()
		{
			...
			this.lblError.Text = "";
			Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,119, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
			if (Request.Form["chkID"] != null)
			{
				string [] aSelectedItems = Request.Form["chkID"].Split(",".ToCharArray());
				foreach(string c in aSelectedItems)
					try{Libreria.Configuracion.Empleados.Eliminar(c);}
					catch (Exception ex)
					{	
