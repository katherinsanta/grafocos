...
{
	public class FormasPago : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack) 
			{
				if (this.VerificarPermisos())
					Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,26, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
					this.Visualizar();
			}
			else
			{
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
                this.AgregarAccion(ref htmlAcciones, "FormaPago.aspx", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1267, Global.Idioma));
			if (eliminar)
                this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1285, Global.Idioma));
		...
		private void Visualizar()
		{
			try
			{
				...
				if(objFormasPago.Count == 0)
				{
					Libreria.Configuracion.FormasPago.AddDefault();
					objFormasPago = new Libreria.Configuracion.FormasPago();
				}

				Results.DataSource = objFormasPago;
				Results.DataBind();
			}
			catch(Exception ex)
			{
				this.SetError( Libreria.Configuracion.PalabrasIdioma.Traduccion(1761, Global.Idioma)	+" " + ex.Message,false);
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,29, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.FormasPago.Eliminar(Convert.ToInt16(aSelectedItems[i]));
