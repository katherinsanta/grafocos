...
{
	public class Estaciones : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				if (!this.Page.IsPostBack) 
				{
					...
					{
						Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,66, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
						this.Visualizar();
					}
				}
				else
		...
		private bool VerificarPermisos()
		{
			...
			int estaciones = objEstaciones.Count;
			objEstaciones = null;

			//Configuración de Acciones
			string htmlAcciones = "";
			if (nuevo && estaciones == 0)
                this.AgregarAccion(ref htmlAcciones, "Estacion.aspx", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1825 , Global.Idioma));
			if (eliminar && estaciones > 0)
                this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1388, Global.Idioma));
		...
		{
			Libreria.Configuracion.Estaciones objEstaciones = new Libreria.Configuracion.Estaciones();
			Results.DataSource = objEstaciones;
			Results.DataBind();
			objEstaciones = null;
		}
		#endregion
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,69, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.Estaciones.Eliminar(Convert.ToInt16(aSelectedItems[i]));
					}
					catch (Exception ex)
					{
