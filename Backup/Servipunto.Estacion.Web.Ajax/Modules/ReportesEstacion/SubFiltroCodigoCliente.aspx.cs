...
{
	public class SubFiltroCodigoCliente : System.Web.UI.Page
	{
		...
		private void Buscar()
		{
			Servipunto.Estacion.Libreria.Clientes objClientes = new Servipunto.Estacion.Libreria.Clientes();
			if(	objClientes.Buscar(Request.QueryString["Buscar"]) != 0 )
			{
				Results.DataSource = objClientes;
				Results.DataBind();
				this.SeccionResultados.Visible = true;
			}
			else
			{
				SetError("No se encontro ningun registro.");
			}
		}
		#endregion
	}
