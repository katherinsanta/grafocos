...
{
	public class FiltroVentasGratis : System.Web.UI.Page
	{
		...
		private bool VerificarPermisos()
		{
			...
			permiso = ((Libreria.Usuario)Session["Usuario"]).Permisos.Verificar(modulo, opcion, "Ventas Gratis Periodo Tiempo");
		...
		private void EjecutarReporte()
		{
			try
			{
				Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,337, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
				DataDynamics.ActiveReports.ActiveReport report = null;
				//El reporte va a ser detallado
                report = new Servipunto.Estacion.Web.Modules.Rpt.VentasGratisDetallado(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text));
		...
		{
            #region traduccion
            this.lblReporte.Text = "<b>" + Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(175, Global.Idioma) + "</b>";
