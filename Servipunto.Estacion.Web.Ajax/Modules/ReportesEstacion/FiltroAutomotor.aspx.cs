...
{
	public class FiltroAutomotor : System.Web.UI.Page
	{
		...
		private bool Validar()
		{
			...
			if(!this.cbTodas.Checked && this.txtAutomotor.Text == "")
			{
                this.SetError(Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1809, Global.Idioma), false);
				return false;
			}
			...
			if (this.Validar())
			{
				try
				{
					...
					if (this.TipoReporte.SelectedValue == "Detallado")
                        report = new Servipunto.Estacion.Web.Modules.Rpt.VentasAutomotorDetallado(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text), this.txtAutomotor.Text, this.cbTodas.Checked);
					else if (this.TipoReporte.SelectedValue == "Resumido")
                        report = new Servipunto.Estacion.Web.Modules.Rpt.VentasAutomotorResumido(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text), this.txtAutomotor.Text, this.cbTodas.Checked);
					else if (this.TipoReporte.SelectedValue == "SoloVenta")
                        report = new Servipunto.Estacion.Web.Modules.Rpt.VentasAutomotorSoloVenta(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text), "", this.txtAutomotor.Text, 1);
					else if (this.TipoReporte.SelectedValue == "Identificador")
                        report = new Servipunto.Estacion.Web.Modules.Rpt.VentasAutomotorIdentificador(Convert.ToDateTime(txtFechaInicial.Text), Convert.ToDateTime(txtFechaFin.Text), "", this.txtAutomotor.Text, 1);					
		...
		{
            #region traduccion
            this.lblReporte.Text = "<b>" + Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(311, Global.Idioma) + "</b>";
            ...
            this.TipoReporte.Items.Insert(0, new ListItem(Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(335, Global.Idioma), "SoloVenta"));
            this.TipoReporte.Items.Insert(0, new ListItem(Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(336, Global.Idioma), "Identificador"));
