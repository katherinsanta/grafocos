...
{
	public class Poss : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack) 
			{
				...
				{
					Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,104, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
					this.Visualizar();
				}
			}
			else
		...
		private bool VerificarPermisos()
		{
			...
			string htmlAcciones = "";
			if (nuevo)
                this.AgregarAccion(ref htmlAcciones, "Pos.aspx?IdCapturador=" + Request.QueryString["IdCapturador"], Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1397, Global.Idioma));
			if (eliminar)
                this.AgregarAccion(ref htmlAcciones, "javascript:document.forms[0].submit();", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1401, Global.Idioma));
			if(Request.QueryString["IdCapturador"] != null && Request.QueryString["IdCapturador"] != "")
                this.AgregarAccion(ref htmlAcciones, "Capturadores.aspx", Servipunto.Estacion.Libreria.Configuracion.PalabrasIdioma.Traduccion(1000, Global.Idioma));
			else if(Request.QueryString["IdIsla"] != null && Request.QueryString["IdIsla"] != "")
		...
		private void Visualizar()
		{
			...
			try
			{
				Libreria.Configuracion.POSs objPoss;

				if(Request.QueryString["IdCapturador"] != null && Request.QueryString["IdCapturador"] != "")
					objPoss = new Libreria.Configuracion.POSs(0, int.Parse(Request.QueryString["IdCapturador"]));
				else if((Request.QueryString["IdIsla"] != null && Request.QueryString["IdIsla"] != ""))
					objPoss = new Libreria.Configuracion.POSs(int.Parse(Request.QueryString["IdIsla"]),0);	
				else
					objPoss = new Libreria.Configuracion.POSs(0, 0);

				Results.DataSource = objPoss;
				Results.DataBind();
			}
			catch(Exception ex)
			{
		...
		private void Eliminar()
		{
			if (Request.Form["chkID"] != null)
			{
				...
				Session["IdSuceso"] = Servipunto.Auditoria.Libreria.Sucesos.Adicionar(Servipunto.Estacion.Libreria.Aplicacion.Conexion ,107, ((Libreria.Usuario)Session["Usuario"]).IdUsuario.ToString(), "U").ToString();
				...
				for (i = 0; i <= aSelectedItems.Length-1; i++) 
				{
					try
					{
						Libreria.Configuracion.POSs.Eliminar(Convert.ToInt16(aSelectedItems[i]));
					}
					catch (Exception ex)
					{
